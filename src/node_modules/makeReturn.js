...
async function makeReturn ({ data = {}, theme = {} }, protocol) {
  ...
  if (output) { result = makeTxReturn(css, output) } // CHANGE
  else { // view and pure
    opts.transaction = transaction
    result = makeTxReturn(css, JSON.stringify(getReturnData(opts), null, 2))
  }
...
function makeTxReturn (css, data) {
  return bel`<div class=${css.txReturnValue}>${data}</div>`
}
const variables = {}
const classes = csjs`
.txReturnItem {
